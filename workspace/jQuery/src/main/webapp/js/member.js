$(function(){
	$('#join').submit(function(){
		...
		if(!$('input[name="gender"]').is(':checked')){
			alert('성별을 입력하세요');
			//document.form1.gender[0].checked=true; //남자 선택
			//$('input[name="gender"]:eq(1)').attr('checked',true); //여자 선택  //attr 문자 취급
			$('input[name="gender"]:eq(1)').prop('checked',true);	//객체 취급
			return false;
		}
